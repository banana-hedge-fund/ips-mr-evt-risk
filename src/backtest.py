...
Упрощения (режим ascii/ускоренный, ИПС):
...
ADAPTIVE_MAP = {
    "R_N": RegimeParams(1.0, 1.5, 0.05, True),
    "R_E": RegimeParams(0.5, 2.0, 0.03, True),
    "R_X": RegimeParams(0.1, 3.0, 0.015, False),
}
BASELINE = RegimeParams(1.0, 1.5, 0.05, True)


@dataclass
class BTParams:
    window: int = 60          # окно z-score (мин)
    fee: float = 0.0004       # комиссия на сторону (4 bps, taker)
    rv_window: int = 60       # окно реализованной волатильности
    rv_baseline: int = 1440   # длинное окно базы волатильности (сутки)
    var_window: int = 1440    # окно для rolling VaR
    var_q: float = 0.99
    viol_window: int = 240     # окно доли VaR-нарушений
...
def compute_signals(df: pd.DataFrame, p: BTParams) -> pd.DataFrame:
    ...
    # rolling historical VaR и доля нарушений
...
def backtest(df: pd.DataFrame, adaptive: bool, p: BTParams = BTParams()) -> dict:
    ...
    pos = 0.0          # текущая позиция (знак*размер)
    entry_price = np.nan
    ...
    for t in range(len(idx)):
        ...
        # ежедневный чекпоинт: закрыть всё в конце суток (начало нового дня)
        ...
        if pos != 0.0:
            # стоп-лосс по движению цены против позиции
        ...
        # открытие новой позиции (mean-reversion: против отклонения)
        if pos == 0.0 and rp.allow_new:
            if zt <= -rp.k_entry:
    ...
    # PnL бара: позиция предыдущего бара * доходность текущего
    ...
    return {
        ...
        "pnl": pnl,
        "position": pos_s,
        "regime": pd.Series(regime_arr, index=idx),
        "signals": sig,
