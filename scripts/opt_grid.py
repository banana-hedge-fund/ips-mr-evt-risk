...
Сетка: window×k_entry×d_stop×cooldown. Train=2024, Test=2026. Результат — results/opt_chunk_*.csv.
"""
...
WINDOWS=[30,60,120,240]; KENTRY=[1.5,2.0,2.5,3.0]; DSTOP=[0.015,0.03,0.05]; COOL=[5,10,30]
...
def run_combo(c):
    ...
    for (s,m),df in DATA.items():
        ...
        r=bt.backtest(df,False,bt.BTParams(window=w,cooldown=cd),bt.RegimeParams(1.0,k,d,True))
        sm=metrics.summarize(r["equity"],r["pnl"])
        rows.append(dict(window=w,k_entry=k,d_stop=d,cooldown=cd,symbol=s,month=m,year=yr,
...
if __name__=="__main__":
    ...
    combos=list(itertools.product(WINDOWS,KENTRY,DSTOP,COOL))
